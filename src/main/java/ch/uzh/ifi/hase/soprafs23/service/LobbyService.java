...
public class LobbyService {
    ...
    public void gameSettings(Long lobbyCode, int roundToEndGame, int playerSize) {
        ...
        if (lobby.getPlayers().size() > playerSize || lobby.getPlayers().size() > 6) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "There are already more players in Lobby");
        }
        else {
